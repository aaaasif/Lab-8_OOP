...
public class GourmetCoffee  {
	...
	public void displayCatalog() {
		...
		if (size == 0) {
			...
			for (Iterator<Product> i = catalog.getProductsIterator(); i.hasNext();) {
	...
	public void displayOrder() {
		...
		if (size == 0) {
			...
			for (Iterator<OrderItem> i = currentOrder.getItemsIterator(); i.hasNext();) {
	...
	public void displayNumberOfOrders(Product product) {
			...
			for (Iterator<Order> i = sales.getOrdersIterator(); i.hasNext();) {
			  Order order = i.next();
			  for (Iterator<OrderItem> j = order.getItemsIterator(); j.hasNext();) {
				if (j.next().getProduct().equals(product)) {
				  count++;
				  break; 
				}
			  }
	...
	public void displayTotalQuantityOfProducts() {
			...
			for (Iterator<Order> i = sales.getOrdersIterator(); i.hasNext();) {
			  ...
			  for (Iterator<OrderItem> j = order.getItemsIterator(); j.hasNext();) {
				...
				Product product = catalog.getProduct(orderItem.getProduct().getCode()); 
				int quantity = productQuantities.get(product);
				productQuantities.put(product, quantity + orderItem.getQuantity());
			  }
			}
		  
			for (Product product : productQuantities.keySet()) {
			  int quantity = productQuantities.get(product);
			  stdOut.println(product.getCode() + " " + quantity);
			}
		  }
